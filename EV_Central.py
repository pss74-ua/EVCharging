...
socket_port = None
panel_refresh_event = threading.Event()
    ...
    for message in consumer:
        try:
            ...
            with cp_states_lock:
                ...
                if cp and cp['status'] == 'IDLE': # 'IDLE' es 'Activado'
                    # AUTORIZAR
...
def handle_monitor_client(conn, addr):
    ...
    try:
            ...
            if msg.get('type') == 'MONITOR_STATUS':
                ...
                with cp_states_lock:
                    if status == 'FAULTED':
                        ...
                        # El monitor reporta recuperación
                        if cp_states[cp_id]['status'] == 'FAULTED':
        ...
        if cp_id:
            # Si un monitor se desconecta, el CP pasa a estado DESCONECTADO
            with cp_states_lock:
                if cp_id in cp_states:
...
def main():
    ...
    try:
        while True:
            choice = input() # Espera el input del admin
